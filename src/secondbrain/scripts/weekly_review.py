...
STOPWORDS = frozenset([
    ...
    "is", "it", "its", "itself", "just", "let", "like", "me", "might", "more", "most", "my",
    "myself", "no", "nor", "not", "now", "of", "off", "on", "once", "only", "or", "other",
    "our", "ours", "ourselves", "out", "over", "own", "re", "same", "she", "should", "so",
    ...
    "yourself", "yourselves", "also", "going", "really", "think", "went", "today",
    "tomorrow", "yesterday", "need", "new", "one", "two", "three",
])
...
def _collect_week_data(daily_dir: Path, start: date, end: date, week_key: str) -> WeekData:
    ...
    recurring = [
        word for word, count in all_word_counts.most_common() if len(days_with_word.get(word, set())) >= 3
    ][:10]
